...
let state = { reports: [], employees: [], payroll: [], departments: [], filteredReports: [], currentProfile: null };
...
function displayProfile(profile) {
  ...
  $("profileDepartment").textContent = emp.course || "-";
  $("profileSalary").textContent = emp.salary ? `₹${parseFloat(emp.salary).toLocaleString()}` : "-";
...
async function loadProfile() {
  const employeeId = $("profileEmployeeId").value;
  if (!employeeId) {
...
export function initReportsController() {
  ...
  const profileEmployeeId = $("profileEmployeeId");
  if (profileEmployeeId) {
    profileEmployeeId.addEventListener("keypress", (e) => {
